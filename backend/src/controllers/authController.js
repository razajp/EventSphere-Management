...
export const loginUser = async (req, res) => {
  try {
    ...
    res.json({ message: "Login successful", token, user: { id: user._id, name: user.name, email: user.email, role: user.role, profileCompleted: user.profileCompleted, status: user.status } });
  } catch(err){ res.status(500).json({ message: err.message }); }
};
